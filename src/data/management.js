export const managementContent = {
  meta: {
    eyebrow: 'Filarmoniya',
    title: 'Rahbariyat',
    description:
      "Filarmoniyaning strategik va ijodiy yo'nalishini belgilaydigan rahbariyat tarkibi. Rasmiy qabul kunlari va aloqa ma'lumotlari.",
    backHref: '/about',
    backLabel: 'Filarmoniya haqida',
    category: 'management',
  },
  members: [
    {
      slug: 'abdulla-nazarov',
      fullname: 'Abdulla Nazarov',
      position: 'Bosh direktor',
      department: "Rahbariyat",
      schedule: 'Dushanba, Payshanba: 15:00 – 17:00',
      contact: {
        phone: '+998 71 233 45 67',
        email: 'rahbar@filarmoniya.uz',
      },
      address: "Toshkent sh., Mustaqillik maydoni, 1-uy, 2-qavat, 201-xona",
      social: [
        { platform: 'Telegram', href: 'https://t.me/filarmoniya_uz' },
        { platform: 'Facebook', href: 'https://facebook.com/filarmoniya.uz' },
      ],
      description:
        "Abdulla Nazarov O'zbekiston Davlat Filarmoniyasining bosh direktori sifatida muassasaning strategik, moliyaviy va ma'muriy faoliyatini boshqaradi. 25 yildan ortiq madaniyat sohasidagi tajribasi davomida xalqaro festivallar va milliy konsert dasturlarini muvaffaqiyatli amalga oshirgan.",
      img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuAq0bTve7jECeanDAWU9h-3AoNrbH3PFUvjC0zXrq8D6caKwDzcbRGNio7WC5kglAn-PlxOrAHMK5okZoQqfOpM0qSglzWOLK-DZz7nqHHKsnx61S78yI6IX79BtM8YMzuiKy5yhX_wR-_aAc-9s-GB0j3qsez9etW9GJ-Dt-qg2hOGcKEJDz5jTBi27XKd9UtXpYf2ZkDaXY33uKvLAjhvBUbBGplcVTOBU9U_wOfzEri5ux2bYP3M3tWtXFe3sDvbQhcVv-_CAg',
    },
    {
      slug: 'dilshoda-rahimova',
      fullname: 'Dilshoda Rahimova',
      position: "Bosh direktor o'rinbosari — Badiiy rahbar",
      department: "Rahbariyat",
      schedule: "Seshanba, Juma: 10:00 – 13:00",
      contact: {
        phone: '+998 71 233 45 68',
        email: 'badiiy@filarmoniya.uz',
      },
      address: "Toshkent sh., Mustaqillik maydoni, 1-uy, 2-qavat, 205-xona",
      social: [
        { platform: 'Instagram', href: 'https://instagram.com/filarmoniya' },
        { platform: 'Telegram', href: 'https://t.me/filarmoniya_uz' },
      ],
      description:
        "Dilshoda Rahimova badiiy rahbar lavozimida konsert dasturlari va repertuar shakllantirish bo'yicha ijodiy qarorlarni boshqaradi. Konservatoriyani qizil diplom bilan tugatgan, Yevropa va Osiyo davlatlarida malaka oshirgan.",
      img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDaeF2gL94Y8InZrivIopQ4HGWzKUjC-0gjlRVVyyNG_jRH1pPgJpf_geSJ0B-3DDz54S5u0soUgylVMzBCqlJIm4AVfHAKfejISBRXK5TtXe0vBlcscdOG3ORoKcIpSHqhUbk3p8H35mS_jhh2g9uAZIL8GolhoodDxPTXNwsrAu8vvakvbKqJ68MgGuzPF9k7Yj9-3BCeuE1HFrquigNZoTOPgaZDWD6wqT9cZbKQ90aNizsKCdiUrkNTA8VTFd-GyWXbWDZbxw',
    },
    {
      slug: 'rustam-yusupov',
      fullname: 'Rustam Yusupov',
      position: 'Bosh dirijyor',
      department: 'Rahbariyat',
      schedule: 'Chorshanba: 14:00 – 16:00',
      contact: {
        phone: '+998 71 233 45 69',
        email: 'dirijyor@filarmoniya.uz',
      },
      address: "Toshkent sh., Mustaqillik maydoni, 1-uy, 3-qavat, 312-xona",
      social: [
        { platform: 'Instagram', href: 'https://instagram.com/r.yusupov' },
        { platform: 'YouTube', href: 'https://youtube.com/@yusupov' },
      ],
      description:
        "Rustam Yusupov simfonik orkestrning bosh dirijyori sifatida har mavsumda 40 dan ortiq konsertni boshqaradi. Uning ijrosida Mahler, Shostakovich va o'zbek bastakorlarining asarlari yangi qiyofada ochiladi.",
      img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDKCHWE6Q1VGxZVPH5Y_5JVeNkWxA9zBwV0-JhGLix1eGqR_NNZNBTtvRxc1jXuMQ8_CjA1KRYzVgLEYdVpo5YWRwocBllLT8Vca8181A78kzGQAQZjchf33VDTucztxoEAeI2_xO8F3vcfNpPjPtzll7hLOClr_PooGrIDWnv0sU9Ton0WXLkTy0q2EEvon2JowwiMzbOJXBrbItAHYw7y8e1bGsgobEC9vZ5ZY-EKmp9oF28t0lQbDgOI1E1QpuDdlmkohQIvGQ',
    },
    {
      slug: 'gulnora-tursunova',
      fullname: "Gulnora Tursunova",
      position: "Bosh direktor o'rinbosari — Moliyaviy ishlar",
      department: 'Rahbariyat',
      schedule: 'Dushanba, Chorshanba: 09:00 – 12:00',
      contact: {
        phone: '+998 71 233 45 70',
        email: 'moliya@filarmoniya.uz',
      },
      address: "Toshkent sh., Mustaqillik maydoni, 1-uy, 1-qavat, 108-xona",
      social: [],
      description:
        "Gulnora Tursunova muassasaning byudjet, hisobot va xo'jalik faoliyatini muvofiqlashtiradi. 18 yillik moliyaviy boshqaruv tajribasiga ega.",
      img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuB8GFukjEXrNHV7uEsEvG7MrVPE9PPWzu5ACT3DrNiGeVtvN2lrgbdFbYc8hLcxhat0SPD2pZcH-sUzLfcam_xg0MaoeujskGGFGxH_Ef-DY2Y9ov3YEq-8vkW01pQ0vrN4cz9-spKgSgTPrSdgtActBWOoUQ_6jsU2ZeO9t_pt2DZUPvb8EX44JpywpBs7AIHG6eAtK_npPHm862QkE-COP_fXl1eT7crowpkEmR3-emoPxXBRAgBfPqDk0s2UdWy0buFMG1W99g',
    },
  ],
};
